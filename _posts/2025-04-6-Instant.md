...
Since we have this password it has to be thee for something but I can not find where. Without anything to go by I run Linpeas to try to find hidden files or other points of interest. Turns out it was in plain sight in the `/opt` directory. 
...
The file seems to be bse64 encoded but after decoding it it does not produce anything meaningful and doing a `file` on the new file gives us `data` which means its not a known file type. Since this file was inside the `Solar-PuTTY` directory I try to investigate what this app does.
