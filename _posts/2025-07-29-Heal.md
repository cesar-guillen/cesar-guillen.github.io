...
```
#### Finding the Gemfile
With the capability of reading files I try to find sensitive files that would allow me to get some passwords to log in as ron or ralph. I find this nice [blog post](https://www.geeksforgeeks.org/ruby/ruby-on-rails-directory-structure/) that shows the directory structure of a ruby on rails installation. For a ruby application the Gemfile usually contains nice information. I try to search for the Gemfile by going back some directories I was able to get a hit two directories down. The Gemfile revealed that a sqlite3 database was being used. 
